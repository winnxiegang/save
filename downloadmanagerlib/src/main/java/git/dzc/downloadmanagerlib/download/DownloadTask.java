...
//                onCompleted();
            ...
            if (response != null && response.isSuccessful()) {
                ...
                if (responseBody != null) {
                    ...
                    //获取真实的fileName
                    if (!TextUtils.isEmpty(response.header("Content-Disposition"))) {
                        fileName = response.header("Content-Disposition");

                        fileName = fileName.substring(fileName.indexOf("filename=") + "filename=".length(), fileName.length());
...
//            Log.d("onDownloadComplete", dbEntity.toString());
        ...
        if (toolSize == completedSize) {
            ...
            String s = MD5.md5sum(new File(saveDirPath + fileName));
            Log.i("split","md5sum+"+s);
            if (MD5.md5Check(split[3].substring(0,split[3].indexOf(".")), new File(saveDirPath + fileName))) {
                downloadStatus = DownloadStatus.DOWNLOAD_STATUS_SUCCESS;
            } else {
                downloadStatus = DownloadStatus.DOWNLOAD_STATUS_ERROR;
            }*/
        }
...
//        Log.d("onDownloadComplete2", dbEntity.toString());
        ...
        Log.d("downloadStatus", "downloadStatus="+downloadStatus);
    ...
    private void onAlreadyDownload() {
        for (DownloadTaskListener listener : listeners) {
            listener.onAlreadyDownload(this);
        }
    }
