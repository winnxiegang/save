...
//        be = (int) ((w / STANDARD_WIDTH + h/ STANDARD_HEIGHT) / 2);
    ...
    public static int compress(String filePath, String dstPath, int reqWidth, int reqHeight) {
        Bitmap smallBitmap = getScaleBitmap(filePath, reqWidth, reqHeight);
        if (smallBitmap == null) {
            return 0;
        } else {
            try {
                File filePic = new File(dstPath);
                if (!filePic.exists()) {
